...
void Test_FindFileByEvent_V40(int iUserID)
{
    NET_DVR_SEARCH_EVENT_PARAM_V40 param = {0};
    param.wMajorType = 100;
    param.struStartTime.dwYear = 2016;
    param.struStartTime.dwMonth = 8;
    param.struStartTime.dwDay = 5;
    
    param.struEndTime.dwYear = 2016;
    param.struEndTime.dwMonth = 8;
    param.struEndTime.dwDay = 6;
    
    param.uSeniorParam.struStreamIDParam.struIDInfo.dwSize = sizeof(param.uSeniorParam.struStreamIDParam.struIDInfo);
    memcpy(param.uSeniorParam.struStreamIDParam.struIDInfo.byID, "2C347018D4D54D269C4FD0E261BB415A", strlen("2C347018D4D54D269C4FD0E261BB415A"));
    int lSearchHandle = NET_DVR_FindFileByEvent_V40(iUserID, &param);
    if(lSearchHandle >= 0)
    {
        NET_DVR_SEARCH_EVENT_RET_V40 struEventRet = {0};
        int lSearchState = -1;
        int nCount = 0;
        while (1)
        {
            lSearchState = NET_DVR_FindNextEvent_V40(lSearchHandle, &struEventRet);
            if (lSearchState != NET_DVR_FILE_SUCCESS && lSearchState != NET_DVR_ISFINDING)
            {
                break;
            }
            ...
            {
                NSLog(@"NET_DVR_FILE_SUCCESS MajorType[%d]", struEventRet.wMajorType);
                nCount++;
            }
        }
        NSLog(@"find next event over, search state[%d],count[%d]", lSearchState, nCount);
